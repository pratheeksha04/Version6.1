...
def TempOnTnGraph(condition_value='None'):
    if condition_value == '2':
        traces = []
        for i in range(13):
            trace = go.Scatter(x=ipm.rpm, y=tmpTn[i], mode='markers',marker=dict(size=5))
            traces.append(trace)
...
def TempOnPower(condition_value='None'):
    ...
    if condition_value == '2':
        traces = []
        for i in range(13):
            trace = go.Scatter(x=ipm.rpm, y=tmpPwr[i], mode='markers', marker=dict(size=6))
            traces.append(trace)
...
def IdIqControlMapByTemp(condition_value='None'):
    ...
    p5000 = np.where(a12efftab[:, 0] == 5000)[0]
    n5000 = np.where(a12efftab[:, 10] == 5000)[0]
    p10000 = np.where(a12efftab[:, 0] == 10000)[0]
    n10000 = np.where(a12efftab[:, 10] == 10000)[0]
...
def IPMModelingTabButtonDown():
    ...
    global ipm,ev,ct,mv,mp,gt,tmpTn,tmpPwr,tmpId,tmpIq
    curflag = 2
    ...
    tmpTn,tmpPwr,tmpId,tmpIq =ipmclass.temperaturemap(arr,len(ipm.rpm),ipm.Temp,ipm.IPMstatus,ipm.Tn,ipm.Pe,ipm.Id,ipm.Iq)
...
def DrivepatternmodelingTabButtonDown():
    ...
    if dpflag == 1:
        ...
        Max_rpm=sum(dp.werpm)
        socPe=sum(dp.AccPe)
        socPbtt=sum(dp.AccPbatt)
        ploss_td=sum(dp.Ploss)
        cop_loss=sum(dp.Pcu)
        Iron_loss=sum(dp.Pfe)
        Inverter_loss=sum(dp.Pinv)
        stray_loss=sum(dp.Pstr)
        friction_loss=sum(dp.Pf)
        Windage_loss=sum(dp.Pw)
        effc=sum(dp.n)
        wnum = dp.num
        dpflag = 1
        if evsimflag == 0:
...
#         ipmobj.initial = ipm.initial
#         ipmobj.increment = ipm.increment
#         ipmobj.init(mot,DTtab,DTnum,PLmat,tnflag,peflag,const)
#         ipmobj.losscalc(crussim, inv, igbt, Temp, Flag, len(ipmobj.rpm))
...
#     fig = go.Figure()
#     fig.add_trace(go.Bar(x=list(range(1, 7)), y=bttcon, name='BTT (kWh/10km)', marker_color='blue'))
...
#     fig.add_trace(go.Scatter(x=list(range(1, 7)), y=crusdist, name='Cruising Distance', mode='lines+markers', marker=dict(color='yellow', size=10), line=dict(color='red')))
#     fig.update_layout(showlegend=False)
#     # fig.show()
#     return fig
...
def data_tabg1(condition_value='None'):
    ...
    # bttcon=1.9289909251
    # Extract and process ecdata
    ecdata = datatable.iloc[2:15, 1:3].values
   
    # Convert the data to numeric types
    ecdata = ecdata.astype(float)
    ...
    # Plot encom data
    fig.add_trace(go.Scatter(x=ecdata[:, 0], y=encom, mode='markers', name='Encom', marker=dict(color='rgb(52, 196, 235)', symbol='circle', size=10)))
   
    # Add text labels for each point
    for i, txt in enumerate(datatable.iloc[2:15, 0]):
        fig.add_annotation(x=ecdata[i, 0], y=encom[i], text=txt, showarrow=False,font=dict(size=10,color='white'))
   
    # Plot BTTchargeEditField.Value and bttcon
    fig.add_trace(go.Scatter(x=[btt['charge']], y=np.real([dp.bttcon]), mode='markers', name='BTTcon', marker=dict(color='red', symbol='triangle-up', size=10)))
    fig.add_annotation(x=btt['charge'], y=np.real(dp.bttcon), text='Simulated model', showarrow=False,font=dict(size=10,color='white'))
   
    # Add a trendline
    z = np.polyfit(ecdata[:, 0], encom, 1)
    p = np.poly1d(z)
    trendline_x = np.linspace(0, 120, 100)
    ...
    fig.update_layout(
                    ...
                    paper_bgcolor="rgba(0,0,0,0)",  # Set paper background color to transparent
                    plot_bgcolor="rgba(0,0,0,0)",
                    xaxis=dict(title='kW', showline=False, showgrid=False,zeroline=False),
                    yaxis=dict(title='Encom',showline=False, showgrid=False,zeroline=False,),
                    #yaxis2=dict(title='Encom',showline=False, showgrid=False,zeroline=False,side='right'),
                    showlegend=False,
                    autosize=True,   # Set plot background color to transparent
                    yaxis_title_font=dict(color="white"),  # Set y-axis title color to white
                    xaxis_tickfont=dict(color="white"),     # Set x-axis tick labels color to white
...
def data_tabg2(condition_value='None'):
...
def data_tabg4(condition_value='None'):
...
def data_tabg5(condition_value='None'):
    datatable = pd.read_excel("InputTableFile.xlsx", sheet_name="Data")
...
def data_tabg6(condition_value='None'):
    datatable = pd.read_excel("InputTableFile.xlsx", sheet_name="Data", header=None)
...
table_body= dmc.TableTbody(children=[
    ...
    dmc.TableTr([
            dmc.TableTd("Max Torque"),
            dmc.TableTd(0,id="tab-maxtorque"),
            dmc.TableTd("Nm"),
        ]),
    dmc.TableTr([
            dmc.TableTd("Max Efficiency %"),
            dmc.TableTd(0,id="tab-maxeff"),
            dmc.TableTd("%"),
        ]),  
    dmc.TableTr([
            dmc.TableTd("Max Positive Efficiency %"),
...
ipm_tab = dmc.Flex(
    ...
    #     dmc.Divider(variant="solid"),
        ...
        dmc.Flex([
            dmc.Text("MaxSpeed", c="white",size="sm",style={"width": "100px"}),
            dmc.NumberInput(value=10000,w=100,hideControls="True",id="EffMaxSpeed",disabled=True,),
            dmc.Text("MaxSpeed (Max LUT)rpm",size="sm", c="white",style={"width": "100px"}),
            dmc.NumberInput(value=10000,w=100,hideControls="True",style={"width": "100px"},id="MaxSpeedRpmEditField",disabled=True)
        ...
        dmc.Flex([
            ...
            dmc.Text("Ini. temp", c="white",size="sm",style={"width": "100px"}),
            dmc.NumberInput(value=65,id="InitempEditField",w=100,hideControls="True",style={"width": "100px"},disabled=True,)
        ], direction={"base": "row", "sm": "row","lg":"row"},gap={"base": "sm", "sm": "sm","lg":"sm"},),
        dmc.Flex([
            ...
            dmc.Text("Room Temp", c="white",size="sm",style={"width": "100px"}),
...
driving_tbody=dmc.TableTbody(
    children=[
        ...
        dmc.TableTr([
            dmc.TableTd("SOC Total: Acc(Pe) w/o Ploss"),
            dmc.TableTd(-2372.8219444,id="td-socPe"),
            dmc.TableTd("kW/1800sec"),
        ]),
        dmc.TableTr([
            dmc.TableTd("SOC Total: Acc(Pbtt) w/Ploss"),
        ...
        dmc.TableTr([
            dmc.TableTd("Copper loss"),
            dmc.TableTd(262.60881132,id="td-coploss"),
            dmc.TableTd("kW"),
        ]),
        dmc.TableTr([
            dmc.TableTd("Iron loss"),
            dmc.TableTd(663.26034752,id="td-ironloss"),
            dmc.TableTd("kW"),
        ]),
        dmc.TableTr([
            dmc.TableTd("Inverter loss"),
            dmc.TableTd(229.66154512,id="td-invloss"),
            dmc.TableTd("kW"),
        ]),
        dmc.TableTr([
            dmc.TableTd("Stray loss"),
            dmc.TableTd(17.902185547,id="td-strayloss"),
            dmc.TableTd("kW"),
           
        ]),  
        dmc.TableTr([
            dmc.TableTd("Friction loss"),
            dmc.TableTd(95.970688,id="td-fricloss"),
            dmc.TableTd("kW"),
        ]),
        dmc.TableTr([
            dmc.TableTd("Windage loss"),
            dmc.TableTd(95.970688,id="td-winloss"),
            dmc.TableTd("kW"),
        ]),
        dmc.TableTr([
            dmc.TableTd("Efficiency"),
            dmc.TableTd(95.970688,id="td-effic"),
            dmc.TableTd("kW"),])            
    ]
)
paramter_header = [
    html.Thead(html.Tr([html.Th("Zones",colSpan=2,style={"textAlign": "center","backgroundColor":"#7d2e8f", "borderRadius": "5px","color":"black","height":"50px","fontWeight": "normal" }),
                        html.Th(" ",style={"color":""}),
...
dr_row1 = html.Tr([html.Td("Low", style={"color":"white","textAlign": "center", "borderRadius": "5px"}),  
                  #dcc.Input( type="number", value=0.004,className="rounded-inputval",style={"backgroundColor":"#a6a6a6"}
                html.Td(dcc.Input( type="number", value=1.2,className="rounded-inputval",id="LowEditField",style={"backgroundColor":"#a6a6a6"})), 
                ...
                html.Td("Pcu",style={"color":"white","textAlign": "center", "borderRadius": "5px"}), 
...
dr_row2 = html.Tr([html.Td("Medium" ,style={"color":"white","textAlign": "center", "borderRadius": "5px"}),  html.Td(dcc.Input( type="number", value=1,id="MediumEditField",className="rounded-inputval",style={"backgroundColor":"#a6a6a6"})), 
                ...
                html.Td("Pfe",style={"color":"white","textAlign": "center", "borderRadius": "5px"}), html.Td(dcc.Input( type="number",id="Pfeflag", value=2,className="rounded-inputval",style={"backgroundColor":"#a6a6a6"})), 
                html.Td(""), html.Td("")
                ],style={"padding": "5px","marginTop":"5px"})#dcc.Input(id="v-input", type="number", value=350, className="rounded-input")

dr_row3 = html.Tr([html.Td("High",style={"color":"white","textAlign": "center", "borderRadius": "5px"}),  html.Td(dcc.Input( type="number", id="HighEditField", value=1,className="rounded-inputval",style={"backgroundColor":"#a6a6a6"})), 
                ...
                html.Td("Pstr",style={"color":"white","textAlign": "center", "borderRadius": "5px",}), html.Td(dcc.Input( type="number",id="Pstrflag", value=1,className="rounded-inputval",style={"backgroundColor":"#a6a6a6"})), 
                html.Td(""), html.Td("")
                ],style={"padding": "5px", "marginBottom": "10px"})#dcc.Input(id="v-input", type="number", value=350, className="rounded-input")

dr_row4= html.Tr([html.Td("Ex-High",style={"color":"white","textAlign": "center", "borderRadius": "5px"}),  html.Td(dcc.Input( type="number", id="ExHighEditField", value=1,className="rounded-inputval",style={"backgroundColor":"#a6a6a6"})), 
                ...
                html.Td("Pf",style={"color":"white","textAlign": "center", "borderRadius": "5px"}), html.Td(dcc.Input( type="number",id="Pfflag", value=1,className="rounded-inputval",style={"backgroundColor":"#a6a6a6"})), 
                html.Td(""), html.Td("")
                ],style={"padding": "5px", "marginBottom": "10px"})#dcc.Input(id="v-input", type="number", value=350, className="rounded-input")
...
dr_row5= html.Tr([html.Td("Vehicle Dynamics", colSpan=2,style={"color":"white", "textAlign": "center", "borderRadius": "5px"}),  
                 ...
                 html.Td(""), 
                html.Td(""), html.Td(""),
                html.Td("Pw",style={"color":"white","textAlign": "center", "borderRadius": "5px"}), html.Td(dcc.Input( type="number",id="Pwflag", value=1,className="rounded-inputval",style={"backgroundColor":"#a6a6a6"})), 
                html.Td(""), html.Td("")
                ],style={"padding": "5px", "marginBottom": "10px"})#dcc.Input(id="v-input", type="number", value=350, className="rounded-input")
...
dr_row6= html.Tr([html.Td( colSpan=2 ),
                html.Td(""), html.Td(""), 
                html.Td(""), html.Td(""),
                html.Td("Pinv",style={"color":"white","textAlign": "center", "borderRadius": "5px"}), html.Td(dcc.Input( type="number",id="Pinvflag", value=2,className="rounded-inputval",style={"backgroundColor":"#a6a6a6"})), 
                html.Td(""), html.Td("")
                ],style={"padding": "5px", "marginBottom": "10px"})#dcc.Input(id="v-input", type="number", value=350, className="rounded-input")
...
dr_row8= html.Tr([  html.Td("Tn-50", style={"color":"white","textAlign": "center", "borderRadius": "5px"}), 
                ...
                html.Td(""),html.Td(dmc.Button("CLTC energy consumption(kwh/10km)Vs Batt cap.", variant="light", style={"width": "100%"}),colSpan=4,style={"textAlign": "center",}),
                ],style={"padding": "5px", "marginBottom": "10px"})#dcc.Input(id="v-input", type="number", value=350, className="rounded-input")
...
dr_row9= html.Tr([   html.Td("Tn-100", style={"color":"white","textAlign": "center", "borderRadius": "5px"}), 
                ...
                html.Td(""),html.Td(dmc.Button("Battery Derating series", variant="light", style={"width": "100%"}),colSpan=4,style={"textAlign": "center",}),
                ],style={"padding": "5px", "marginBottom": "10px"})#dcc.Input(id="v-input", type="number", value=350, className="rounded-input")
...
dr_row10= html.Tr([  html.Td("Tn-200", style={"color":"white","textAlign": "center", "borderRadius": "5px"}), 
                ...
                html.Td(""),html.Td(dmc.Button("Motor Specifications", variant="light" ,style={"width": "100%"}),colSpan=4,style={"textAlign": "center",}),
                ],style={"padding": "5px", "marginBottom": "10px"})#dcc.Input(id="v-input", type="number", value=350, className="rounded-input")
...
dr_row11= html.Tr([ html.Td("Tn-300", style={"color":"white","textAlign": "center", "borderRadius": "5px"}), 
                ...
                html.Td(""),html.Td(dmc.Button("80 kW Leaf", variant="light",style={"width": "100%"}),colSpan=4,style={"textAlign": "center",}),
                ],style={"padding": "5px", "marginBottom": "10px"})#dcc.Input(id="v-input", type="number", value=350, className="rounded-input")
        ...
        # ),
            ...
            gap="xs",),
                dmc.Flex([
                    ...
                    dmc.Flex(
                        [
                            dmc.Text("Gravity", c="white",size="sm",style={"width": "80px"}),
                            dmc.NumberInput(value=9.81,w=60,id="GravityEditField",hideControls="True",disabled=True),
                            dmc.Text("m..", c="white",style={"width": "20px"}),
       
                            dmc.Text("Rolling Resistance", c="white" ,size="sm",style={"width": "120px","marginLeft":"15px"}),
                    ...
                    dmc.Flex(
                        [
                            dmc.Text("Road Grade", c="white",size="sm",style={"width": "80px"}),
                    ...
                    dmc.Flex([
                        ...
                        dmc.Button("Id vs Iq", id="btn-cursd6", variant="light",style={"marginLeft":"5px", "whiteSpace": "normal", "wordBreak": "break-word", "textAlign": "center", "height": "auto", "lineHeight": "normal"}),
                        ...
                        dmc.Modal(id="modal-cursd1",centered=True,zIndex=10000,children=[ dcc.Graph(id='cursd1'),],),
                        dmc.Modal(id="modal-cursd2",centered=True,zIndex=10000,children=[ dcc.Graph(id='cursd2'),],),
                        dmc.Modal(id="modal-cursd3",centered=True,zIndex=10000,children=[ dcc.Graph(id='cursd3'),],),
                        dmc.Modal(id="modal-cursd4",centered=True,zIndex=10000,children=[ dcc.Graph(id='cursd4'),],),
                        dmc.Modal(id="modal-cursd5",centered=True,zIndex=10000,children=[ dcc.Graph(id='cursd5'),],),
                        dmc.Modal(id="modal-cursd6",centered=True,zIndex=10000,children=[ dcc.Graph(id='cursd6'),],),
    ...
    [
        dmc.Flex(
            [
                ...
                dmc.Drawer(title="Ploss Torque",id="Tn-drawer",padding="xs",size="100%",children=Next_Tn),
                dmc.Modal(
                    title="WARNING",
...
@app.callback(
    ...
    [Input("next-link","children"),Input("next-link","disable_n_clicks"),Input('dtA1', 'children'), Input('dtB1', 'children'),Input('dtLd1', 'children'), Input('dtLq1', 'children'), Input('pmd1', 'children'), Input('pmq1', 'children'), Input('dta1', 'children'),
     ...
     Input('speed1','children'),Input('speed2','children'),Input('speed3','children'),Input('speed4','children'),Input('speed5','children'),Input('speed6','children'),Input('InitialEditField','value'),Input('IncrementEditField','value'),
     prevent_initial_call=True,
     allow_duplicate=True
...
def update_output(nextlink,disable_n_clicks,dtA1, dtB1, dtLd1, dtLq1, pmd1, pmq1, dta1, 
                  ...
                  speed1,speed2,speed3,speed4,speed5,speed6,InitialEditField,IncrementEditField
                  ):
    # resetab()
    global mot,inv,gear,tire,vhcl
    global DTtab,DTnum,ipm,a12
    global type,hdcalf,hqcalf,const,peflag,tnflag,Mode
    global ldopt,lqopt,pmdopt
    a12={}
...
def reload_wltcdata(disable_n_clicks,GravityEditField, frEditField, VWindEditField,
    ...
    if tab == "drive-pattern" and disable_n_clicks == None :
        ...
        figured6= IdIqAvatarA12(condition_value='1')
...
def PLossanalysisbyTorqueTabButtonDown(tab, active_cell, clicks, click_counts):
    ...
    TnScale_init = round(max(dp.Tn), -1)
...
def toggle_modal(n_clicks, opened):
    figure12 = IdIqAvatarA12(condition_value='2')
    return not opened,figure12
#full screen tab2
...
def LosstypeDropDownValueChanged(tab ,selected_value):
    ...
    # # Interpolate the ploss values over the grid
    # Zq = griddata((x, y), ploss, (Xq, Yq), method='cubic')

    # # Apply the mask to the interpolated Z values
    # Zq[~in_polygon] = np.nan

    # # Define contour levels
    # levels = np.linspace(np.nanmin(ploss), np.nanmax(ploss), 10)
    ...
    #     # colorbar=dict(title='Ploss'),
    #     # contours=dict(
    #     #     start=np.nanmin(ploss),
    #     #     end=np.nanmax(ploss),
    #     #     size=(np.nanmax(ploss) - np.nanmin(ploss)) / 10
    #     # )
    # )

    # # Combine the scatter plot and contour map
    # fig = go.Figure(data=[scatter])

    # # Customize layout and axis labels
    # fig.update_layout(
    #     title="Contour Plot with Scatter Points",
    #     xaxis_title="RPM",
    #     yaxis_title="Tn",
    #     plot_bgcolor='black',
    #     paper_bgcolor='black',
    ...
    #     yaxis=dict(showgrid=False, color='#4DBEEE')
    # )
    ...
    # # Create an empty list to store modified z values
    # z_list = []

    # # Load the data from the "inp.xlsx" file
    # df = pd.read_excel("inp.xlsx")
    # df = np.array(df)
    # df = df.reshape(x.shape)
    # z = griddata((x, y),ploss, (x, y), method='linear')
    # z[~df] = np.nan
    # z_list.append(z)        

    # Create the figure and add scatter and contour
